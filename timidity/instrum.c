									...
									Sint32 vib_control_ratio)
{
	...
	return (Sint32)(FSCALE((double)(vib_control_ratio)*SWEEP_TUNING, SWEEP_SHIFT) / (double)(song->rate * sweep));
...
static Sint32 convert_vibrato_rate(MidiSong *song, Uint8 rate)
{
	...
	return (VIBRATO_RATE_TUNING * song->rate) /
...
#define READ_CHAR(thing)                     \
	if (1 != SDL_RWread(rw, &tmpchar, 1, 1)) \
		goto fail;                           \
	thing = tmpchar;
#define READ_SHORT(thing)                     \
	if (1 != SDL_RWread(rw, &tmpshort, 2, 1)) \
		goto fail;                            \
	thing = SDL_SwapLE16(tmpshort);
#define READ_LONG(thing)                     \
	if (1 != SDL_RWread(rw, &tmplong, 4, 1)) \
		goto fail;                           \
