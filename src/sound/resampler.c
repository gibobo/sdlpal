...
ALIGNED static float *cubic_lut = NULL;
...
void resampler_init(void)
{
    ...
    sinc_lut = UTIL_calloc(SINC_SAMPLES + 1, sizeof(float));
    window_lut = UTIL_calloc(SINC_SAMPLES + 1, sizeof(float));
    ...
    x = 0.0;
    cubic_lut = UTIL_calloc(CUBIC_SAMPLES, sizeof(float));
...
{
    resampler *r = (resampler *)UTIL_malloc(sizeof(resampler));
    ...
    r->last_amp = 0;
    r->accumulator = 0;
    r->buffer_in = (float *)UTIL_calloc(resampler_buffer_size * 2, sizeof(float));
    r->buffer_out = (float *)UTIL_calloc(resampler_buffer_size + SINC_WIDTH * 2 - 1, sizeof(float));
    return r;
}
